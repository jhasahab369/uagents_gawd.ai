...
# Edges define transitions between the states in the dialogue

# Transition to initiate the session
init_session = create_edge(
    name="Initiate session",
    description="Every dialogue starts with this transition.",
    target="user",
    observable=True,  # Indicates this transition is observable
    parent=default_state,
    child=init_state,
)

# Transition to reject the dialogue
...
# Transition to start the actual dialogue (move to chatting state)
...
# Transition for continuing the dialogue (staying in chatting state)
...
class ChitChatDialogue(Dialogue):
    ...
    def __init__(
        ...
        # Initialize the dialogue with its name, version, states (nodes), transitions (edges), storage, and cleanup interval
        super().__init__(
    ...
    # Handlers for different state transitions
